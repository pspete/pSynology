...
Describe $FunctionName {
	...
	InModuleScope $ModuleName {
		...
		Context "Default" {
			...
			It "does not throw" { { $InputObj | Save-SYNOFSFile } | Should Not throw }

			It "sends request" {

				$InputObj | Save-SYNOFSFile

				Assert-MockCalled Invoke-WebRequest -Times 1 -Exactly -Scope It

			}

			It "reports error if thumbnail not saved" {

				{
					Mock Set-Content -MockWith { throw }

					$InputObj | Save-SYNOFSFile -erroraction Stop
