...
Describe $FunctionName {
	...
	InModuleScope $ModuleName {
		...
		Context "Default" {
			...
			It "does not throw" { { $InputObj | Update-SYNOFSFavorite } | Should Not throw }

			It "sends request" {

				$InputObj | Update-SYNOFSFavorite
			...
			It "reports error if count parameter values do not match" {

				{
					$InputObj = [PSCustomObject]@{
						...
						"path" = "SomeFolderPath", "AnotherFolderPath", "SomeOtherFolderPath"
					}
