		...
		Context "Default" {
			...
			BeforeEach {
				...
				$InputObj = [PSCustomObject]@{
					...
					"taskid"      = "SomeID"
					"folder_path" = "SomeFolderPath"
				}

			}

			It "does not throw" { { $InputObj | New-SYNOFSFolder } | Should Not throw }

			It "sends request" {

				$InputObj | New-SYNOFSFolder

				Assert-MockCalled Invoke-WebRequest -Times 1 -Exactly -Scope It

			}

			It "reports error if count parameter values do not match" {

				{
					$InputObj = [PSCustomObject]@{
						"Name"        = "SomeName", "SomeOtherName"
						"folder_path" = "SomeFolderPath", "AnotherFolderPath", "SomeOtherFolderPath"
					}
